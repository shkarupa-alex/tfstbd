...
class TestCaseFeatures(tf.test.TestCase):
    def test_features(self):
        ...
        result_no_case = self.evaluate(no_case.to_tensor(default_value=False))
        self.assertAllEqual(expected_no_case, result_no_case.tolist())

        result_lower_case = self.evaluate(lower_case.to_tensor(default_value=False))
        self.assertAllEqual(expected_lower_case, result_lower_case.tolist())

        result_upper_case = self.evaluate(upper_case.to_tensor(default_value=False))
        self.assertAllEqual(expected_upper_case, result_upper_case.tolist())

        result_title_case = self.evaluate(title_case.to_tensor(default_value=False))
        self.assertAllEqual(expected_title_case, result_title_case.tolist())

        result_mixed_case = self.evaluate(mixed_case.to_tensor(default_value=False))
