...
def build_model(h_params, ngram_keys):
    ...
    outputs = TextVectorization(ngram_keys, name='ngrams_lookup')(word_ngrams)
    ...
    outputs = tf.keras.layers.Concatenate(name='word_features')([outputs, word_feats])
    if 'lstm' == h_params.seq_core:
        for i, units in enumerate(h_params.lstm_units):
            outputs = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(
                units, return_sequences=True), name='sequence_features_{}'.format(i))(outputs)
    elif 'causal' == h_params.tcn_padding:
        outputs = tf.keras.layers.Bidirectional(TemporalConvNet(
            h_params.tcn_filters, h_params.tcn_ksize, h_params.tcn_drop))(outputs)
    else:
        outputs = TemporalConvNet(h_params.tcn_filters, h_params.tcn_ksize, h_params.tcn_drop, 'same')(outputs)
    outputs = tft.keras.layers.ToDense(pad_value=0., mask=True, name='dense_logits')(outputs)

    # TODO https://www.kaggle.com/christofhenkel/keras-baseline-lstm-attention-5-fold/

    outputs_space = tf.keras.layers.Dense(1, activation='sigmoid', name='space')(outputs)
    outputs_token = tf.keras.layers.Dense(1, activation='sigmoid', name='token')(outputs)
    outputs_sentence = tf.keras.layers.Dense(1, activation='sigmoid', name='sentence')(outputs)
