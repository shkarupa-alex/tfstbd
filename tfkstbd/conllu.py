...
def has_text(parsed):
    ...
    return parsed.metadata is not None and \
           'text' in parsed.metadata and \
           parsed.metadata['text'] is not None and \
           len(parsed.metadata['text'])
...
def repair_spaces(parsed):
    ...
    for i, t in enumerate(meaning_i):
        ...
        space = extract_space(current_token)
    ...
    for last_i in {-1, meaning_i[-1]}:
        if 'misc' in parsed[last_i] and parsed[last_i]['misc'] is not None:
            parsed[last_i]['misc']['SpaceAfter'] = 'No'
            if 'SpacesAfter' in parsed[last_i]['misc']:
                del parsed[last_i]['misc']['SpacesAfter']
        else:
            parsed[last_i]['misc'] = OrderedDict({'SpaceAfter': 'No'})
